...
if(strlen($error)==0){
	...
	if($result) {
        ...
        $qry = "SELECT id FROM composers WHERE fname= '$fname' AND lname= '$lname' ORDER BY id DESC LIMIT 1"; //finds composer id, last id number
		$result = @mysql_query($qry);
		...
		if($result) {
			while($row = @mysql_fetch_array($result)){
				...
				$qry = "INSERT INTO composer_description (description, composer_id) VALUES($description,$composer_id)";
				$result = @mysql_query($qry);
				if($result) {
				}else{
					$error = "Failed to add composer description.";
					break;
				}
			}
		}else{
			$error = "Failed to get the composer id.";
		}
