...
<div id='body'>
	<div id='left'>
		<div id='welcome'>
			<h2 class="guilded"><span>Browse Library</span></h2>
		</div>
	</div>
</div>
...
if ($flip == FALSE || EMPTY($flip)) {   // No previous alter, ascending
	...
	echo "<TH><A href=\"" . $PHP_SELF . "?sort=arranger1$extend\">Arranger</A></FONT></TH>\n";
	...
	echo "<TH><A href=\"" . $PHP_SELF . "?sort=title$extend\">Composition</A></FONT></TH>\n";
        echo "<TH><A href=\"" . $PHP_SELF . "?sort=arranger$extend\">Arranger</A></FONT></TH>\n";
...
for ($i = 0; $i < $rownum; $i++) { //the meat of the table
	...
	if (empty($pieces[$i][2])) // NULL CHECK, maybe there's no arranger
		$pieces[$i][2] = "&nbsp";
	echo "<TD align = \"center\">$color" . $pieces[$i][2] . "</FONT></TD>\n";
